...
/**
 * Returns a robust YYYY-MM-DD string shifted natively to the requested timezone.
 * Avoids en-CA locale assumptions bridging Serverless Edge differences.
 */
export function toLocalYYYYMMDD(date: Date, timezone: string): string {
...
export function todayDate(timezone = 'Asia/Kolkata'): string {
  return toLocalYYYYMMDD(new Date(), timezone);
}
