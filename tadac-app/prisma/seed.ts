/**
 * Seed file — populates the database with:
 * 1. A default user with settings
 * 2. Sample interview questions across all topic groups
 * 3. Sample curated projects (URL Shortener, Booking System)
 * 4. Sample motivational quotes (stored in a JSON config)
 *
...
async function main() {
  ...
  // ─── Curated projects ──────────────────────────────────────────────────────
  const urlShortener = await prisma.project.upsert({
  ...
  console.log(`✓ Project: ${urlShortener.name} (${urlShortener.stages.length} stages)`);

  const bookingSystem = await prisma.project.upsert({
  ...
  console.log(`✓ Project: ${bookingSystem.name} (${bookingSystem.stages.length} stages)`);
